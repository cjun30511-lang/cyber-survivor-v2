...
async function main() {
  ...
  // Wait 4 seconds for lobby
  setTimeout(async () => {
    console.log('🎮 Lobby loaded. Attempting transition to BattleScene...');
    
    // Evaluate transition script
    send('Runtime.evaluate', {
      expression: `(() => {
        if (window.game && window.game.scene && window.game.scene.keys.MenuScene) {
          console.log("Triggering scene change to BattleScene...");
